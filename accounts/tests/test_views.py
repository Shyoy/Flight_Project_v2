...
class RegisterFormTest(TestDataMixin, TestCase):
    ...
    def test_form_valid_post(self):
        ...
        self.assertIn(self.customers_group,
                      response.wsgi_request.user.groups.all())
    ...
    def test_form_invalid_post(self):
        ...
        self.assertEqual(str(
            messages[0]), 'User Registration failed Make sure the data you entered is valid !')
...
class AdminRegisterTest(TestDataMixin, TestCase):
    ...
    def setUp(self):
        self.c = Client()
        self.c.login(username=self.super_user.username,
                     password='testsuperuser2')
    ...
    def test_register_admin_redirect(self):
        ...
        self.assertEqual(
            str(messages[0]), f'Create a profile for User "{self.user_admin}" !')
...
class AirlineRegisterTest(TestDataMixin, TestCase):

    def setUp(self):
        self.c = Client()
        self.c.login(username=self.user_admin.username,
                     password='adminpassword1')

    def test_get(self):
        response = self.c.get(reverse('airline_register'))
        # self.c.login(username='testuser1', password='testpassword1')
        user_form = response.context.get('user_form')
        airline_form = response.context.get('airline_form')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(airline_form and user_form)
